...
This project asks whether global commodity-price changes and short-run volatility improve the one-month-ahead prediction of high-conflict months in Sub-Saharan Africa beyond information contained in recent conflict history. The objective is predictive rather than causal and is formulated as a supervised binary-classification problem at the country-month level.
...
Five course-aligned classifiers are compared: Logistic Regression, k-nearest neighbors, Decision Tree, AdaBoost, and a feed-forward multilayer perceptron. The dataset is divided chronologically into training, validation, and held-out test periods to avoid look-ahead bias. Model selection is based primarily on validation F1, with precision, recall, accuracy, and confusion matrices also reported.

The central empirical comparison is between a conflict-history baseline and the same model augmented with commodity variables. Success is therefore not defined by finding a positive commodity effect, but by assessing whether commodity dynamics provide stable incremental predictive information out of sample.
